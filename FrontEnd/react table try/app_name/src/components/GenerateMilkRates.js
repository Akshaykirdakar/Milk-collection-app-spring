...
const GenerateMilkRates = () => {
  ...
  const handleGenerate = async (group) => {
    ...
    try {
      ...
      setMessages((prev) => ({
        ...prev,
        [key]: { type: 'success', text: 'Milk rates generated successfully!' },
      }));
    } catch (err) {
      setMessages((prev) => ({
        ...prev,
        [key]: { type: 'error', text: 'Failed to generate rates.' },
      }));
    }
  };
  ...
  return (
    <Box className="add-user-container">
      <Typography variant="h5" gutterBottom>Generate Milk Rates</Typography>
          ...
          return (
            <Paper key={idx} elevation={3} sx={{ p: 3, mb: 3 }}>
              ...
              <Box display="flex" alignItems="center" gap={2} mt={2}>
                ...
                />
                <Button variant="contained" onClick={() => handleGenerate(group)}>Generate</Button>
              </Box>
